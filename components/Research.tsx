...
export function Research({ section, index, research }: ResearchProps) {
  return (
    <Section section={section} index={index}>
      {research.intro ? (
        <Reveal className="mb-12 max-w-prose text-body text-ink-soft">
          <p>{research.intro}</p>
        </Reveal>
      ) : null}
        ...
        <Reveal delay={0.08} className="mt-14">
          ...
          <ul className="mt-5 space-y-6">
            {research.outputs.map((output) => (
              <li
                key={output.id}
                className="max-w-3xl rounded-sheet border border-ink/10 bg-paper-raised p-6 shadow-card sm:p-8"
              >
                <p className="font-mono text-label uppercase text-pen">{output.role}</p>

                <h4 className="mt-4 font-serif text-h3 italic">
                ...
                <div className="mt-6 flex flex-wrap items-center gap-x-4 gap-y-3 border-t border-ink/10 pt-5">
                  <p className="font-mono text-micro uppercase tracking-[0.14em] text-ink-soft">
                    {output.date ? (
                      output.dateISO ? (
                        <time dateTime={output.dateISO}>{output.date}</time>
                      ) : (
                        output.date
                      )
                    ) : null}
                    {output.date && output.venueShort ? (
                      <span aria-hidden="true" className="mx-2.5 text-ink/25">
                        /
                      </span>
                    ) : null}
                    {output.venueShort}
                  ...
                  {output.award ? (
                    <p className="rounded-full border border-pen/30 bg-pen/5 px-3 py-1 font-mono text-[0.6875rem] uppercase tracking-[0.14em] text-pen">
                      {output.award}
                    </p>
                  ) : null}
                </div>
